...
test('family allowance is one 10% RMV benefit and removes the former salary cap conclusion', () => {
  ...
  assert.match(source, /No se multiplica por cada hijo/i);
});
test('AFP vs ONP comparator is installed and audited enhancements load from frontend startup', () => {
  assert.match(source, /afpOnpComparison/); assert.match(source, /onp:\s*13/);
  assert.match(frontend, /pension-labor-enhancements\.js/); assert.match(frontend, /loadAuditedModuleEnhancements\(\)/);
});
test('AFP projection does not convert fund into a guaranteed pension',()=>{assert.match(source,/function calcProyAfp/);assert.match(source,/Se retiró la conversión automática/);assert.doesNotMatch(source,/fondo \* 0\.04 \/ 12/);});
