...
test('bank reconciliation and rate converter have functional workspaces',()=>{assert.match(s,/ptConcilBanc/);assert.match(s,/Saldo banco ajustado/);assert.match(s,/ptConversorTasas/);assert.match(s,/Math\.pow\(1\+annual,1\/n2\)/);});
...
test('standalone reconciliation and rate converter are reachable from navigation',()=>{assert.match(s,/ensureNav\('concil_banc'/);assert.match(s,/ensureNav\('conversor_tasas'/);});
