...
test('export balance separates SFMB from drawback',()=>{assert.match(s,/SFMB y drawback son mecanismos distintos/);assert.doesNotMatch(s,/limiteDrawback/);});
...
test('Amazon benefit has eligibility safety note',()=>{assert.match(s,/Ley 27037/);assert.match(s,/no se determinan solo por departamento o domicilio/);});
