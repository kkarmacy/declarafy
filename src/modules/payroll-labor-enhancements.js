...
function calcCas(){const salary=Number(document.getElementById('cas_sueldo')?.value||0),type=document.getElementById('cas_tipo')?.value||'',months=Number(document.getElementById('cas_meses')?.value||12),box=document.getElementById('casResult');if(!box)return;if(!(salary>0&&type==='publico')){box.style.display='none';return}box.style.display='';box.innerHTML='<div class="res-table"><table><tr><td>Remuneración mensual informada</td><td>'+money(salary)+'</td></tr><tr><td>Meses informados</td><td>'+months+'</td></tr></table>'+note('Se retiró la escala ficticia de “tasa CAS” de 0% a 15% y el supuesto aporte CAS calculado sobre el sueldo. El régimen CAS (D.Leg. 1057 y modificaciones) exige revisar vínculo, entidad, beneficios, régimen pensionario y reglas vigentes; no existe una tasa CAS general por tramo salarial.')+'</div>';}
...
function install(){global.calcNomina=calcNomina;global.calcHorasExtras=calcHorasExtras;global.calcTRegistro=calcTRegistro;global.calcEssaludSenati=calcEssaludSenati;global.calcCas=calcCas;addBenefitsWorkspace();}
