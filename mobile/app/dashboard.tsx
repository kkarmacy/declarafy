...
const tools = [
 ...
 {title:'Calculadoras',desc:'IGV y cálculos tributarios',route:'/calculators'},
 {title:'Multas y TIM',desc:'Consulta y calcula',route:'/tim'},
 {title:'Fraccionamiento',desc:'Simula y conoce tus opciones',route:'/installments'},
];
export default function Dashboard(){
 return <SafeAreaView style={s.page}><ScrollView contentContainerStyle={s.content}>
  <View style={s.top}><View><Text style={s.hello}>Hola 👋</Text><Text style={s.subtitle}>Tu aliado tributario siempre</Text></View><TouchableOpacity onPress={()=>router.push('/profile')}><Text style={s.profile}>Mi perfil</Text></TouchableOpacity></View>
  ...
  <Text style={s.section}>Herramientas</Text><View style={s.grid}>{tools.map(item=><ModuleCard key={item.title} title={item.title} description={item.desc} onPress={()=>router.push(item.route as any)}/>)}</View>
